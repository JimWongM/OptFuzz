...
struct BlockReducer: Reducer {
    ...
    func myReduce(_ code: inout Code, with helper: MinimizationHelper) {
        for group in Blocks.findAllBlockGroups(in: code) {
            ...
            case is BeginWhileLoop,
                 ...
                 is BeginRepeatLoop:
                assert(group.numBlocks == 1)
                reduceLoop2(loop: group.block(0), in: &code, with: helper)
    ...
    private func reduceLoop2(loop: Block, in code: inout Code, with helper: MinimizationHelper) {
        assert(loop.begin.isLoop)
        assert(loop.end.isLoop)
    ...
    private func reduceIfElse2(_ group: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        ...
        // First try to remove the entire if-else block but keep its content.
        if helper.tryNopping2(group.excludingContent().map({ $0.index }), in: &code) {
            return
        }
    ...
    private func reduceBeginSwitch2(_ group: BlockGroup, in code: inout Code, with helper: MinimizationHelper) {
        ...
        for block in blocks {
            ...
            if block.begin.op is BeginSwitchDefaultCase { continue }
            // (3) Try to remove the cases here.
            helper.tryNopping2(Array(block.head...block.tail), in: &code)
        }
    }
