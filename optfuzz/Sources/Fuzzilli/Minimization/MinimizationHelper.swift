...
class MinimizationHelper {
    ...
    func test2(_ code: Code) -> Bool {
        ...
        var stillHasAspects = false
        performOnFuzzerQueue {
            //fuzzer.evaluator.clearMyBitmap()
            let execution = fuzzer.execute(Program(with: code), withTimeout: fuzzer.config.timeout * 2)
    ...
    @discardableResult
    func tryReplacing2(instructionAt index: Int, with newInstr: Instruction, in code: inout Code) -> Bool {
        assert(code[index].allOutputs == newInstr.allOutputs)
    ...
    @discardableResult
    func tryInserting2(_ newInstr: Instruction, at index: Int, in code: inout Code) -> Bool {
        // Inserting instructions will invalidate the instructionsToKeep list, so that list must be empty here.
        assert(instructionsToKeep.isEmpty)
    ...
    @discardableResult
    func tryNopping2(instructionAt index: Int, in code: inout Code) -> Bool {
        return tryReplacing2(instructionAt: index, with: nop(for: code[index]), in: &code)
    }
    ...
    @discardableResult
    func tryReplacements2(_ replacements: [(Int, Instruction)], in code: inout Code) -> Bool {
        var originalInstructions = [(Int, Instruction)]()
        var abort = false, result = false
        for (index, newInstr) in replacements {
            if instructionsToKeep.contains(index) {
    ...
    @discardableResult
    func tryNopping2(_ indices: [Int], in code: inout Code) -> Bool {
        var replacements = [(Int, Instruction)]()
        for index in indices {
            replacements.append((index, nop(for: code[index])))
        }
        return tryReplacements2(replacements, in: &code)
    }
...
protocol Reducer {
    ...
    func myReduce(_ code: inout Code, with tester: MinimizationHelper)
}
